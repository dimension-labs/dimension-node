...
fn get_system_contract(system_contract: SystemContractType) -> ContractHash {
    ...
    let contract_hash: ContractHash = {
        let result = {
            ...
            let value = unsafe {
                ext_ffi::dimension_get_system_contract(
...
pub fn create_purse() -> URef {
    ...
    unsafe {
        let ret = ext_ffi::dimension_create_purse(purse_non_null_ptr.as_ptr(), UREF_SERIALIZED_LENGTH);
        if ret == 0 {
...
pub fn get_purse_balance(purse: URef) -> Option<U512> {
    ...
    let value_size = {
        ...
        let ret =
            unsafe { ext_ffi::dimension_get_balance(purse_ptr, purse_size, output_size.as_mut_ptr()) };
...
pub fn transfer_to_account(target: AccountHash, amount: U512, id: Option<u64>) -> TransferResult {
    ...
    let return_code = unsafe {
        ext_ffi::dimension_transfer_to_account(
            target_ptr,
            target_size,
            amount_ptr,
            amount_size,
...
pub fn transfer_from_purse_to_account(
    ...
    let mut maybe_result_value = MaybeUninit::uninit();
    let return_code = unsafe {
        ext_ffi::dimension_transfer_from_purse_to_account(
            source_ptr,
            source_size,
            target_ptr,
            target_size,
...
pub fn transfer_from_purse_to_purse(
    ...
    let (id_ptr, id_size, _bytes4) = contract_api::to_ptr(id);
    let result = unsafe {
        ext_ffi::dimension_transfer_from_purse_to_purse(
            source_ptr,
            source_size,
            target_ptr,
            target_size,
...
pub fn record_transfer(
    ...
    let result = unsafe {
        ext_ffi::dimension_record_transfer(
            maybe_to_ptr,
            maybe_to_size,
            source_ptr,
            source_size,
...
pub fn record_era_info(era_id: EraId, era_info: EraInfo) -> Result<(), ApiError> {
    ...
    let result = unsafe {
        ext_ffi::dimension_record_era_info(era_id_ptr, era_id_size, era_info_ptr, era_info_size)
    };
    if result == 0 {
        Ok(())
    } else {
