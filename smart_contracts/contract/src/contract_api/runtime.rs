...
pub fn ret(value: CLValue) -> ! {
    ...
    unsafe {
        ext_ffi::dimension_ret(ptr, size);
    }
}
...
pub fn revert<T: Into<ApiError>>(error: T) -> ! {
    unsafe {
        ext_ffi::dimension_revert(error.into().into());
    }
}
...
pub fn call_contract<T: CLTyped + FromBytes>(
    ...
    let bytes_written = {
        ...
        let ret = unsafe {
            ext_ffi::dimension_call_contract(
                contract_hash_ptr,
                contract_hash_size,
...
pub fn call_versioned_contract<T: CLTyped + FromBytes>(
    ...
    let bytes_written = {
        let mut bytes_written = MaybeUninit::uninit();
        let ret = unsafe {
            ext_ffi::dimension_call_versioned_contract(
...
pub fn get_caller() -> AccountHash {
    let output_size = {
        ...
        let ret = unsafe { ext_ffi::dimension_get_caller(output_size.as_mut_ptr()) };
        api_error::result_from(ret).unwrap_or_revert();
...
pub fn get_blocktime() -> BlockTime {
    ...
    let bytes = unsafe {
        ext_ffi::dimension_get_blocktime(dest_non_null_ptr.as_ptr());
        Vec::from_raw_parts(
...
pub fn get_phase() -> Phase {
    ...
    unsafe { ext_ffi::dimension_get_phase(dest_non_null_ptr.as_ptr()) };
    let bytes = unsafe {
        Vec::from_raw_parts(
            dest_non_null_ptr.as_ptr(),
...
pub fn has_key(name: &str) -> bool {
    let (name_ptr, name_size, _bytes) = contract_api::to_ptr(name);
    let result = unsafe { ext_ffi::dimension_has_key(name_ptr, name_size) };
    result == 0
}
...
pub fn put_key(name: &str, key: Key) {
    ...
    unsafe { ext_ffi::dimension_put_key(name_ptr, name_size, key_ptr, key_size) };
}
...
pub fn remove_key(name: &str) {
    let (name_ptr, name_size, _bytes) = contract_api::to_ptr(name);
    unsafe { ext_ffi::dimension_remove_key(name_ptr, name_size) }
}
...
pub fn list_named_keys() -> NamedKeys {
    let (total_keys, result_size) = {
        ...
        let ret = unsafe {
            ext_ffi::dimension_load_named_keys(total_keys.as_mut_ptr(), &mut result_size as *mut usize)
        };
        api_error::result_from(ret).unwrap_or_revert();
...
pub fn is_valid_uref(uref: URef) -> bool {
    ...
    let result = unsafe { ext_ffi::dimension_is_valid_uref(uref_ptr, uref_size) };
    result != 0
}
...
fn read_host_buffer_into(dest: &mut [u8]) -> Result<usize, ApiError> {
    let mut bytes_written = MaybeUninit::uninit();
    let ret = unsafe {
        ext_ffi::dimension_read_host_buffer(dest.as_mut_ptr(), dest.len(), bytes_written.as_mut_ptr())
    };
...
pub fn get_call_stack() -> Vec<CallStackElement> {
    let (call_stack_len, result_size) = {
        ...
        let ret = unsafe {
            ext_ffi::dimension_load_call_stack(
...
pub fn print(text: &str) {
    ...
    unsafe { ext_ffi::dimension_print(text_ptr, text_size) }
}
