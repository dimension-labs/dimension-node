...
pub fn get_main_purse() -> URef {
    ...
    let bytes = unsafe {
        ext_ffi::dimension_get_main_purse(dest_non_null_ptr.as_ptr());
        Vec::from_raw_parts(
...
pub fn set_action_threshold(
    ...
    let result = unsafe { ext_ffi::dimension_set_action_threshold(action_type, threshold) };
    if result == 0 {
        Ok(())
    } else {
...
pub fn add_associated_key(account_hash: AccountHash, weight: Weight) -> Result<(), AddKeyFailure> {
    ...
    let result = unsafe {
        ext_ffi::dimension_add_associated_key(
            account_hash_ptr,
            account_hash_size,
            weight.value().into(),
        )
...
pub fn remove_associated_key(account_hash: AccountHash) -> Result<(), RemoveKeyFailure> {
    let (account_hash_ptr, account_hash_size, _bytes) = to_ptr(account_hash);
    let result =
        unsafe { ext_ffi::dimension_remove_associated_key(account_hash_ptr, account_hash_size) };
    if result == 0 {
        Ok(())
    } else {
...
pub fn update_associated_key(
    ...
    // Cast of u8 (weight) into i32 is assumed to be always safe
    let result = unsafe {
        ext_ffi::dimension_update_associated_key(
            account_hash_ptr,
            account_hash_size,
            weight.value().into(),
        )
