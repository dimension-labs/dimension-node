...
pub fn read<T: CLTyped + FromBytes>(uref: URef) -> Result<Option<T>, bytesrepr::Error> {
    ...
    let value_size = {
        ...
        let ret = unsafe { ext_ffi::dimension_read_value(key_ptr, key_size, value_size.as_mut_ptr()) };
...
pub fn write<T: CLTyped + ToBytes>(uref: URef, value: T) {
    ...
    unsafe {
        ext_ffi::dimension_write(key_ptr, key_size, cl_value_ptr, cl_value_size);
    }
}
...
pub fn add<T: CLTyped + ToBytes>(uref: URef, value: T) {
    ...
    unsafe {
        ...
        ext_ffi::dimension_add(key_ptr, key_size, cl_value_ptr, cl_value_size);
    }
}
...
pub fn new_uref<T: CLTyped + ToBytes>(init: T) -> URef {
    ...
    let bytes = unsafe {
        ext_ffi::dimension_new_uref(uref_non_null_ptr.as_ptr(), cl_value_ptr, cl_value_size); // URef has `READ_ADD_WRITE`
        Vec::from_raw_parts(
...
fn create_contract_package(is_locked: bool) -> (ContractPackageHash, URef) {
    ...
    unsafe {
        ext_ffi::dimension_create_contract_package_at_hash(
...
pub fn create_contract_user_group(
    ...
    let value_size = {
        ...
        let ret = unsafe {
            ext_ffi::dimension_create_contract_user_group(
...
pub fn provision_contract_user_group_uref(
    ...
    let value_size = {
        let mut value_size = MaybeUninit::uninit();
        let ret = unsafe {
            ext_ffi::dimension_provision_contract_user_group_uref(
                contract_package_hash_ptr,
                contract_package_hash_size,
                label_ptr,
                label_size,
...
pub fn remove_contract_user_group_urefs(
    ...
    let ret = unsafe {
        ext_ffi::dimension_remove_contract_user_group_urefs(
            contract_package_hash_ptr,
            contract_package_hash_size,
            label_ptr,
            label_size,
...
pub fn remove_contract_user_group(
    ...
    let (label_ptr, label_size, _bytes3) = contract_api::to_ptr(label);
    let ret = unsafe {
        ext_ffi::dimension_remove_contract_user_group(
            contract_package_hash_ptr,
            contract_package_hash_size,
            label_ptr,
            label_size,
...
pub fn add_contract_version(
    ...
    let ret = unsafe {
        ext_ffi::dimension_add_contract_version(
            contract_package_hash_ptr,
            contract_package_hash_size,
...
pub fn disable_contract_version(
    ...
    let result = unsafe {
        ext_ffi::dimension_disable_contract_version(
            contract_package_hash_ptr,
            contract_package_hash_size,
            contract_hash_ptr,
            contract_hash_size,
...
pub fn new_dictionary(dictionary_name: &str) -> Result<URef, ApiError> {
    ...
    let value_size = {
        let mut value_size = MaybeUninit::uninit();
        let ret = unsafe { ext_ffi::dimension_new_dictionary(value_size.as_mut_ptr()) };
        api_error::result_from(ret)?;
        unsafe { value_size.assume_init() }
    };
    let value_bytes = runtime::read_host_buffer(value_size).unwrap_or_revert();
...
pub fn dictionary_get<V: CLTyped + FromBytes>(
    ...
    let value_size = {
        let mut value_size = MaybeUninit::uninit();
        let ret = unsafe {
            ext_ffi::dimension_dictionary_get(
                uref_ptr,
                uref_size,
...
pub fn dictionary_put<V: CLTyped + ToBytes>(
    ...
    let result = unsafe {
        let ret = ext_ffi::dimension_dictionary_put(
            uref_ptr,
            uref_size,
            dictionary_item_key_ptr,
            dictionary_item_key_size,
