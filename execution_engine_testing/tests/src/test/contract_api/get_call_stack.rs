...
use dimension_execution_engine::storage::global_state::in_memory::InMemoryGlobalState;
use dimension_types::{
...
fn assert_invalid_context(builder: &mut InMemoryWasmTestBuilder, depth: usize) {
    if depth == 0 {
        ...
        assert!(matches!(
            error,
            dimension_execution_engine::core::engine_state::Error::Exec(
                dimension_execution_engine::core::execution::Error::InvalidContext
            )
        ));
    }
}
...
mod session {
    use dimension_engine_test_support::{ExecuteRequestBuilder, DEFAULT_ACCOUNT_ADDR};
    use dimension_types::{runtime_args, RuntimeArgs};
...
mod payment {
    ...
    use dimension_execution_engine::shared::wasm;
    use dimension_types::{runtime_args, RuntimeArgs};
