...
pub static CL_TYPES: Lazy<Dependency> = Lazy::new(|| Dependency::new("dimension-types", "1.4.5"));
...
pub mod tests {
    ...
    const PATH_PREFIX: &str = "/execution_engine_testing/cargo_dimension";

    /// Returns the absolute path of `relative_path` where this is relative to "dimension-node".
    /// Panics if the current working directory is not within "dimension-node".
    pub fn full_path_from_path_relative_to_workspace(relative_path: &str) -> String {
        ...
        let index = full_path.find(PATH_PREFIX).unwrap_or_else(|| {
            panic!(
                "test should be run from within dimension-node workspace: {} relative path: {}",
    ...
    // TODO - revert the `#[ignore]` once cargo dimension is updated.
    #[ignore]
    #[test]
