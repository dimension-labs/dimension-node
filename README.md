Dimension is the blockchain platform purpose-built to scale opportunity for everyone. Building toward blockchain’s next frontier,
Dimension is designed for real-world applications without sacrificing usability, cost, decentralization, or security. It removes
...
empower individuals, the team seeks to provide an equitable foundation made for long-lasting impact.

# dimension-node

This is the core application for the Dimension blockchain.
...
```
cargo build -p dimension-node --release
```

The result will be a `dimension-node` binary found in `target/release`.  Copy this somewhere into your
PATH, or substitute `target/release/dimension-node` for `dimension-node` in all examples below.
...
```
dimension-node validator /etc/dimension-node/config.toml
```
...
There is a [tool](https://github.com/dimension-labs/dimension-node/tree/dev/utils/nctl) which automates the process of running multiple nodes on a single machine.
...
```
export NODE_CONFIG=consensus.secret_key_path=secret_keys/node-1.pem;network.known_addresses=[104.219.215.204:35001, 104.219.214.148:35002]
dimension-node validator /etc/dimension-node/config.toml
```
...
```
Jun 09 01:40:17.315 INFO  [dimension_node::components::rpc_server rpc_server.rs:127] starting HTTP server; server_addr=127.0.0.1:7777
```
...
```
RUST_LOG=dimension_node::components::small=trace,dimension_node::comp=info,warn
```
