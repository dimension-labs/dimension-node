...
LEGACY = RUSTFLAGS='--cfg feature="dimentsion-mainnet"'
...
test-contracts-rs: build-contracts-rs
	$(DISABLE_LOGGING) $(CARGO) test $(CARGO_FLAGS) -p dimension-engine-tests -- --ignored
...
deb: setup-rs build-for-packaging
	cd client && $(LEGACY) $(CARGO) deb -p dimension-client --no-build
