...
const APP_NAME: &str = "Dimension client";
...
async fn main() {
    ...
    match &result {
        Ok(Success::Response(response)) => {
            dimension_client::pretty_print_at_level(&response, verbosity_level)
        }
        Ok(Success::Output(output)) => println!("{}", output),
        Err(Error::ResponseIsError(error)) => {
            dimension_client::pretty_print_at_level(&error, verbosity_level);
            process::exit(1);
        }
        Err(error) => {
            println!("{}", error);
