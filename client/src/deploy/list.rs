...
use dimension_client::{Error, ListDeploysResult};
use dimension_node::rpcs::chain::GetBlockResult;
...
impl<'a, 'b> ClientCommand<'a, 'b> for ListDeploys {
    ...
    async fn run(matches: &ArgMatches<'a>) -> Result<Success, Error> {
        ...
        let result =
            dimension_client::get_block(maybe_rpc_id, node_address, verbosity_level, maybe_block_id)
                .await;
