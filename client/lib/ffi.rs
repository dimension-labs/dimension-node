...
impl AsFFIError for Error {
    fn as_ffi_error(&self) -> casper_error_t {
        match self {
            Error::FailedToParseKey => casper_error_t::DIMENSION_FAILED_TO_PARSE_KEY,
            Error::FailedToParseURef { .. } => casper_error_t::DIMENSION_FAILED_TO_PARSE_UREF,
            Error::FailedToParseInt { .. } => casper_error_t::DIMENSION_FAILED_TO_PARSE_INT,
            Error::FailedToParseTimeDiff { .. } => casper_error_t::DIMENSION_FAILED_TO_PARSE_TIME_DIFF,
            Error::FailedToParseTimestamp { .. } => {
                casper_error_t::DIMENSION_FAILED_TO_PARSE_TIMESTAMP
            }
            Error::FailedToParseUint { .. } => casper_error_t::DIMENSION_FAILED_TO_PARSE_UINT,
            Error::FailedToGetResponse(_) => casper_error_t::DIMENSION_FAILED_TO_GET_RESPONSE,
            Error::FailedToParseResponse(_) => casper_error_t::DIMENSION_FAILED_TO_PARSE_RESPONSE,
            Error::FileAlreadyExists(_) => casper_error_t::DIMENSION_FILE_ALREADY_EXISTS,
            Error::UnsupportedAlgorithm(_) => casper_error_t::DIMENSION_UNSUPPORTED_ALGORITHM,
            Error::ResponseIsError(_) => casper_error_t::DIMENSION_RESPONSE_IS_ERROR,
            Error::InvalidJson(_) => casper_error_t::DIMENSION_INVALID_JSON,
            Error::InvalidRpcResponse(_) => casper_error_t::DIMENSION_INVALID_RPC_RESPONSE,
            Error::FailedSending(_) => casper_error_t::DIMENSION_FAILED_SENDING,
            Error::IoError { .. } => casper_error_t::DIMENSION_IO_ERROR,
            Error::ToBytesError(_) => casper_error_t::DIMENSION_TO_BYTES_ERROR,
            Error::CryptoError { .. } => casper_error_t::DIMENSION_CRYPTO_ERROR,
            Error::InvalidCLValue(_) => casper_error_t::DIMENSION_INVALID_CL_VALUE,
            Error::InvalidArgument { .. } => casper_error_t::DIMENSION_INVALID_ARGUMENT,
            Error::InvalidResponse(_) => casper_error_t::DIMENSION_INVALID_RESPONSE,
            Error::FFISetupNotCalled => casper_error_t::DIMENSION_FFI_SETUP_NOT_CALLED,
            Error::FFIPtrNullButRequired(_) => casper_error_t::DIMENSION_FFI_PTR_NULL_BUT_REQUIRED,
            Error::ConflictingArguments { .. } => casper_error_t::DIMENSION_CONFLICTING_ARGUMENTS,
            Error::DeploySizeTooLarge(_) => casper_error_t::DIMENSION_DEPLOY_SIZE_TOO_LARGE,
            Error::FailedToParseDictionaryIdentifier => {
                casper_error_t::DIMENSION_FAILED_TO_CREATE_DICTIONARY_IDENTIFIER
            }
            Error::FailedToParseStateIdentifier => {
                casper_error_t::DIMENSION_FAILED_TO_PARSE_STATE_IDENTIFIER
            }
        }
    }
}
...
pub extern "C" fn casper_put_deploy(
    ...
    runtime.block_on(async move {
        ...
        casper_error_t::DIMENSION_SUCCESS
    })
}
