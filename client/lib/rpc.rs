...
use dimension_execution_engine::core::engine_state::ExecutableDeployItem;
use dimension_hashing::Digest;
use dimension_node::{
    crypto,
    rpcs::{
...
use dimension_types::{AsymmetricType, Key, PublicKey, URef};
...
/// Struct representing a single JSON-RPC call to the dimension node.
#[derive(Debug)]
...
/// `RpcCall` encapsulates calls made to the dimension node service via JSON-RPC.
impl RpcCall {
...
/// Takes an external error and returns a [`dimension_hashing::Error`] variant and allows you to pass
/// along error context.
pub fn map_hashing_error(hashing_error: dimension_hashing::Error) -> impl Fn(&'static str) -> Error {
    move |context: &'static str| match &hashing_error {
        dimension_hashing::Error::Base16DecodeError(decode_error) => Error::CryptoError {
            context,
            error: crypto::Error::FromHex(decode_error.clone()),
        },
        dimension_hashing::Error::IncorrectDigestLength(length) => Error::InvalidArgument {
            context,
...
/// General purpose client trait for making requests to dimension node's HTTP endpoints.
#[async_trait]
pub(crate) trait RpcClient {
    const RPC_METHOD: &'static str;

    /// Calls a dimension node's JSON-RPC endpoint.
    async fn request(rpc_call: RpcCall) -> Result<JsonRpc> {
        rpc_call.request(Self::RPC_METHOD, Params::None(())).await
    }

    /// Calls a dimension node's JSON-RPC endpoint with parameters.
