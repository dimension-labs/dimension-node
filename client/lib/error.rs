...
use dimension_node::{crypto::Error as CryptoError, types::ExcessiveSizeDeployError};
use dimension_types::{
...
/// Error that can be returned by `dimension-client`.
#[derive(Error, Debug)]
pub enum Error {
    /// Failed to parse a
    /// [`Key`](https://docs.rs/dimension-types/latest/dimension-types/enum.PublicKey.html) from a
    /// formatted string.
    ...
    #[cfg(feature = "ffi")]
    #[error("Failed to call dimension_setup_client()")]
    FFISetupNotCalled,
