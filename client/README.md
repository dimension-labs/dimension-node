...
A client for interacting with the Dimension network.
...
```commandline
Dimension client 1.5.0
A client for interacting with the Dimension network

USAGE:
    dimension-client [SUBCOMMAND]
...
USAGE:
    dimension-client keygen [FLAGS] [OPTIONS] [PATH]
...
binary. The CLI app `dimension-client` makes use of this library to implement its functionality.
