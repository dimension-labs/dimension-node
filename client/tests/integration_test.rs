...
impl MockServerHandle {
    ...
    async fn get_balance(&self, state_root_hash: &str, purse_uref: &str) -> Result<(), Error> {
        dimension_client::get_balance("1", &self.url(), 0, state_root_hash, purse_uref)
            .await
            .map(|_| ())
    }

    async fn get_deploy(&self, deploy_hash: &str) -> Result<(), Error> {
        dimension_client::get_deploy("1", &self.url(), 0, deploy_hash)
            .await
            .map(|_| ())
    }

    async fn get_state_root_hash(&self, maybe_block_id: &str) -> Result<(), Error> {
        dimension_client::get_state_root_hash("1", &self.url(), 0, maybe_block_id)
            .await
            .map(|_| ())
    }

    async fn get_block(&self, maybe_block_id: &str) -> Result<(), Error> {
        dimension_client::get_block("1", &self.url(), 0, maybe_block_id)
            .await
            .map(|_| ())
    }
    ...
    async fn get_item(&self, state_root_hash: &str, key: &str, path: &str) -> Result<(), Error> {
        dimension_client::get_item("1", &self.url(), 0, state_root_hash, key, path)
            .await
            .map(|_| ())
    }
    ...
    async fn query_global_state(
        ...
        dimension_client::query_global_state("1", &self.url(), 0, global_state_params, key, path)
            .await
            .map(|_| ())
    }
    ...
    async fn get_dictionary_item(
        ...
        dimension_client::get_dictionary_item(
            "1",
            &self.url(),
            0,
            state_root_hash,
    ...
    async fn send_deploy_file(&self, input_path: &str) -> Result<(), Error> {
        dimension_client::send_deploy_file("1", &self.url(), 0, input_path)
            .await
            .map(|_| ())
    }

    async fn get_auction_info(&self, maybe_block_id: &str) -> Result<(), Error> {
        dimension_client::get_auction_info("1", &self.url(), 0, maybe_block_id)
            .await
            .map(|_| ())
    }

    async fn get_validator_changes(&self) -> Result<(), Error> {
        dimension_client::get_validator_changes("1", &self.url(), 0)
            .await
            .map(|_| ())
    }
}
...
mod deploy_params {
    ...
    pub fn test_data_valid() -> DeployStrParams<'static> {
        DeployStrParams {
            ...
            ttl: "10s",
            chain_name: "dimension-test-chain-name-1",
            gas_price: "1",
            ..Default::default()
        }
    }
...
mod get_balance {
    ...
    use dimension_client::ValidateResponseError;
    use dimension_types::URefFromStrError;
...
mod get_block {
    use dimension_client::ValidateResponseError;
    use dimension_node::rpcs::chain::{GetBlock, GetBlockParams};
...
mod get_item {
    use dimension_client::ValidateResponseError;
    use dimension_node::rpcs::state::{GetItem, GetItemParams};
...
mod query_global_state {
    use dimension_client::ValidateResponseError;
    use dimension_node::rpcs::state::{QueryGlobalState, QueryGlobalStateParams};
...
mod get_auction_info {
    ...
    use dimension_node::rpcs::state::GetAuctionInfo;
...
mod get_validator_changes {
    ...
    use dimension_node::rpcs::{info::GetValidatorChanges, RpcWithoutParams};
...
mod sign_deploy {
    ...
    fn should_succeed_for_file() {
        ...
        assert!(matches!(
            dimension_client::sign_deploy_file(
                unsigned_file_path.to_str().unwrap(),
    ...
    fn should_fail_with_bad_secret_key_path() {
        ...
        assert!(dimension_client::sign_deploy_file(
            unsigned_file_path.to_str().unwrap(),
            "<this is not a path>",
            "",
            false
...
mod make_transfer {
    ...
    fn should_succeed_for_stdout() {
        assert!(matches!(
            dimension_client::make_transfer(
                "",
                AMOUNT,
                TARGET_ACCOUNT,
                TRANSFER_ID,
...
mod keygen_generate_files {
    ...
    fn should_succeed_for_valid_args_ed25519() {
        ...
        let result = dimension_client::keygen::generate_files(
            path.to_str().unwrap(),
            dimension_client::keygen::ED25519,
            true,
        );
    ...
    fn should_succeed_for_valid_args_secp256k1() {
        ...
        let result = dimension_client::keygen::generate_files(
            path.to_str().unwrap(),
            dimension_client::keygen::SECP256K1,
            true,
        );
    ...
    fn should_fail_for_invalid_output_dir() {
        let path = "";
        let result =
            dimension_client::keygen::generate_files(path, dimension_client::keygen::ED25519, true);
...
mod rate_limit {
    use super::*;
    use dimension_node::types::Timestamp;
