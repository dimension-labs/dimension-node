...
impl From<&Transform> for dimension_types::Transform {
    fn from(transform: &Transform) -> Self {
        match transform {
            Transform::Identity => dimension_types::Transform::Identity,
            Transform::Write(StoredValue::CLValue(cl_value)) => {
                dimension_types::Transform::WriteCLValue(cl_value.clone())
            }
            Transform::Write(StoredValue::Account(account)) => {
                dimension_types::Transform::WriteAccount(account.account_hash())
            }
            Transform::Write(StoredValue::ContractWasm(_)) => {
                dimension_types::Transform::WriteContractWasm
            }
            Transform::Write(StoredValue::Contract(_)) => dimension_types::Transform::WriteContract,
            Transform::Write(StoredValue::ContractPackage(_)) => {
                dimension_types::Transform::WriteContractPackage
            }
            Transform::Write(StoredValue::Transfer(transfer)) => {
                dimension_types::Transform::WriteTransfer(*transfer)
            }
            Transform::Write(StoredValue::DeployInfo(deploy_info)) => {
                dimension_types::Transform::WriteDeployInfo(deploy_info.clone())
            }
            Transform::Write(StoredValue::EraInfo(era_info)) => {
                dimension_types::Transform::WriteEraInfo(era_info.clone())
            }
            Transform::Write(StoredValue::Bid(bid)) => {
                dimension_types::Transform::WriteBid(bid.clone())
            }
            Transform::Write(StoredValue::Withdraw(unbonding_purses)) => {
                dimension_types::Transform::WriteWithdraw(unbonding_purses.clone())
            }
            Transform::AddInt32(value) => dimension_types::Transform::AddInt32(*value),
            Transform::AddUInt64(value) => dimension_types::Transform::AddUInt64(*value),
            Transform::AddUInt128(value) => dimension_types::Transform::AddUInt128(*value),
            Transform::AddUInt256(value) => dimension_types::Transform::AddUInt256(*value),
            Transform::AddUInt512(value) => dimension_types::Transform::AddUInt512(*value),
            Transform::AddKeys(named_keys) => dimension_types::Transform::AddKeys(
                named_keys
                    .iter()
                    .map(|(name, key)| dimension_types::NamedKey {
                        name: name.clone(),
                        key: key.to_formatted_string(),
                    })
                    .collect(),
            ),
            Transform::Failure(error) => dimension_types::Transform::Failure(error.to_string()),
        }
    }
}
...
pub mod gens {
    ...
    use dimension_types::gens::stored_value_arb;
