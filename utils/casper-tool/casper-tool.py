...
@click.option(
    "-b",
    "--basedir",
    help="dimension-node source code base directory",
    type=click.Path(exists=True, dir_okay=True, file_okay=False, readable=True),
    default=os.path.join(os.path.dirname(__file__), "..", ".."),
)
@click.option(
    "--dimension-client",
    help="path to dimension client binary (compiled from basedir by default)",
...
def cli(
    ...
    """Dimension Network creation tool

    Can be used to create new dimension-labs chains with automatic validator setups. Useful for testing."""
    obj = {}
    ...
    if dimension_client:
        obj["dimension_client_argv0"] = [dimension_client]
    else:
        obj["dimension_client_argv0"] = [
            "cargo",
            "run",
            "--quiet",
...
def create_node(
    ...
    if cluster:
        # Set the public address to `dimension-node-XX`, which will resolve to the internal
        # network IP, and use the automatic port detection by setting `:0`.
        config["network"]["public_address"] = "dimension-node-{}:{}".format(n, NODE_PORT)
        config["network"]["bind_address"] = "dimension-node-{}:{}".format(n, NODE_PORT)

        config["network"]["known_addresses"] = [
            "dimension-node-{}.dimension-node.dimension-{}:{}".format(n, network_name, NODE_PORT)
...
def run_client(argv0, *args):
    """Run the dimension client, compiling it if necessary, with the given command-line args"""
