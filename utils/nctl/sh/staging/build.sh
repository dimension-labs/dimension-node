...
function set_stage_binaries()
{
    ...
    if [ "$NCTL_COMPILE_TARGET" = "debug" ]; then
        cargo build --package dimension-node --featuresdimension-mainnet
    else
        cargo build --release --package dimension-node --featuresdimension-mainnet
    fi

    # Set client binary.
    if [ "$NCTL_COMPILE_TARGET" = "debug" ]; then
        cargo build --package dimension-client --featuresdimension-mainnet
    else
        cargo build --release --package dimension-client --featuresdimension-mainnet
    fi
...
function set_stage_files_from_repo()
{
    ...
    # Stage binaries.
    if [ "$NCTL_COMPILE_TARGET" = "debug" ]; then
        cp "$PATH_TO_SOURCE/target/debug/dimension-client" \
           "$PATH_TO_STAGE"
        cp "$PATH_TO_SOURCE/target/debug/dimension-node" \
           "$PATH_TO_STAGE"
        cp "$NCTL_DIMENSION_NODE_LAUNCHER_HOME/target/debug/dimension-node-launcher" \
           "$PATH_TO_STAGE"
    else
        cp "$PATH_TO_SOURCE/target/release/dimension-client" \
           "$PATH_TO_STAGE"
        cp "$PATH_TO_SOURCE/target/release/dimension-node" \
           "$PATH_TO_STAGE"
        cp "$NCTL_DIMENSION_NODE_LAUNCHER_HOME/target/release/dimension-node-launcher" \
           "$PATH_TO_STAGE"
    fi
