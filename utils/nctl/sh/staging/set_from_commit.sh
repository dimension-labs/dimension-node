...
function _main()
{
    ...
    PATH_TO_REPO="$(get_path_to_remotes)/dimension-node"
...
#######################################
# Ensures dimension-node source code is ready to build.
#######################################
function _set_repo()
{
    ...
    if [ ! -d "$PATH_TO_REPO" ]; then
        ...
        git clone "https://github.com/dimension-labs/dimension-node.git" "$PATH_TO_REPO" > /dev/null 2>&1
    fi
