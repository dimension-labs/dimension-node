...
# Base URL: nctl.
_BASE_URL="http://nctl.dimensionlabs.io.s3-website.us-east-2.amazonaws.com"
...
function _main()
{
    ...
    chmod +x ./dimension-client
    chmod +x ./dimension-node
    if [ "${#PROTOCOL_VERSION}" = '3' ]; then
        RC_VERSION=$(./dimension-node --version | awk '{ print $2 }' |  awk -F'-' '{ print $1 }')
        cd ..
