...
function clean_up() {
    ...
    if [ "$EXIT_CODE" == '1' ]; then
        ...
        if [ ! -z "$AWS_ACCESS_KEY_ID" ] && [ ! -z "$AWS_SECRET_ACCESS_KEY" ] && [ "$DRONE" == 'true' ]; then
            ...
            aws s3 cp ./"${DRONE_BUILD_NUMBER}"_nctl_dump.tar.gz s3://nctl.dimensionlabs.io/nightly-logs/ > /dev/null 2>&1
            log "Download the dump file: curl -O https://s3.us-east-2.amazonaws.com/nctl.dimensionlabs.io/nightly-logs/${DRONE_BUILD_NUMBER}_nctl_dump.tar.gz"
            popd
        fi
    fi
...
function do_start_node() {
    ...
    elif [ -f "$(get_path_to_node_storage $NODE_ID)/dimension-net-1/initializer.pid" ]; then
        log "... pid file detected, removing (1.4+)"
        rm -f "$(get_path_to_node_storage $NODE_ID)/dimension-net-1/initializer.pid"
    fi
...
function do_submit_auction_bids()
{
    ...
    log "node-$NODE_ID auction bid submitted -> $BID_AMOUNT DSCC"
