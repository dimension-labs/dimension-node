...
#   Count of nodes to setup (default=5).
#   Path to dimension-client binary.
#   Path to dimension-node binary.
#   Path to dimension-node-launcher binary.
...
function setup_asset_binaries()
{
    ...
    do
        ...
        if [ ! -f "$PATH_TO_BIN/dimension-node-launcher" ]; then
    ...
    do
        ...
        if [ "$(echo $PROTOCOL_VERSION | tr -d '_')" -ge "140" ]; then
            ...
            if [ -f "$PATH_TO_NET/nodes/node-$IDX/storage/$(get_chain_name)/data.lmdb" ]; then
                GLOBAL_STATE_OUTPUT=$("$NCTL_DIMENSION_HOME"/target/"$NCTL_COMPILE_TARGET"/global-state-update-gen \
