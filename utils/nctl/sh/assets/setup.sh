...
PATH_TO_CHAINSPEC=${PATH_TO_CHAINSPEC:-"${NCTL_DIMENSION_HOME}/resources/local/chainspec.toml.in"}
PATH_TO_ACCOUNTS=${PATH_TO_ACCOUNTS:-""}
PATH_TO_CONFIG_TOML=${PATH_TO_CONFIG_TOML:-"${NCTL_DIMENSION_HOME}/resources/local/config.toml"}
...
function _main()
{
    ...
    if [ "$NCTL_COMPILE_TARGET" = "debug" ]; then
        setup_asset_binaries "1_0_0" \
                             "$(get_count_of_nodes)" \
                             "$NCTL_DIMENSION_HOME/target/debug/dimension-client" \
                             "$NCTL_DIMENSION_HOME/target/debug/dimension-node" \
                             "$NCTL_DIMENSION_NODE_LAUNCHER_HOME/target/debug/dimension-node-launcher" \
                             "$NCTL_DIMENSION_HOME/target/wasm32-unknown-unknown/release"
    else
        setup_asset_binaries "1_0_0" \
                             "$(get_count_of_nodes)" \
                             "$NCTL_DIMENSION_HOME/target/release/dimension-client" \
                             "$NCTL_DIMENSION_HOME/target/release/dimension-node" \
                             "$NCTL_DIMENSION_NODE_LAUNCHER_HOME/target/release/dimension-node-launcher" \
                             "$NCTL_DIMENSION_HOME/target/wasm32-unknown-unknown/release"
    fi
