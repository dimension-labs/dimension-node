...
function _upgrade_node() {
    ...
    # Copy dimension-node binary.
    PATH_TO_NODE=$(get_path_to_node "$NODE_ID")
    mkdir -p "$PATH_TO_NODE"/bin/"$PROTOCOL_VERSION"

    if [ "$NCTL_COMPILE_TARGET" = "debug" ]; then
        cp "$NCTL_DIMENSION_HOME"/target/debug/dimension-node "$PATH_TO_NODE"/bin/"$PROTOCOL_VERSION"/
    else
        cp "$NCTL_DIMENSION_HOME"/target/release/dimension-node "$PATH_TO_NODE"/bin/"$PROTOCOL_VERSION"/
    fi
...
function _generate_global_state_update() {
    ...
    "$NCTL_DIMENSION_HOME"/target/"$NCTL_COMPILE_TARGET"/global-state-update-gen $PARAMS \
