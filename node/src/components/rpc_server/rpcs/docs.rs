...
pub(crate) static OPEN_RPC_SCHEMA: Lazy<OpenRpcSchema> = Lazy::new(|| {
    let contact = OpenRpcContactField {
        name: "DimensionLabs".to_string(),
        url: "https://dimensionlabs.io".to_string(),
    };
    let license = OpenRpcLicenseField {
        name: "DimensionLabs Open Source License Version 1.0".to_string(),
        url: "https://raw.githubusercontent.com/DimensionLabs/dimension-node/master/LICENSE".to_string(),
    };
    let info = OpenRpcInfoField {
        version: DOCS_EXAMPLE_PROTOCOL_VERSION.to_string(),
        title: "Client API of Dimension Node".to_string(),
        description: "This describes the JSON-RPC 2.0 API of a node on the Dimension network."
            .to_string(),
        contact,
        license,
    };
    ...
    let server = OpenRpcServerEntry {
        name: "any Dimension Network node".to_string(),
...
/// The main schema for the dimension node's RPC server, compliant with
...
mod tests {
    ...
    fn check_optional_params_fields<T: RpcWithOptionalParams>() -> Vec<SchemaParam> {
        ...
        let license = OpenRpcLicenseField {
            name: "DimensionLabs Open Source License Version 1.0".to_string(),
            url: "https://raw.githubusercontent.com/DimensionLabs/dimension-node/master/LICENSE"
                .to_string(),
        };
        let info = OpenRpcInfoField {
            version: DOCS_EXAMPLE_PROTOCOL_VERSION.to_string(),
