...
#[cfg(test)]
use dimension_execution_engine::core::engine_state::MAX_PAYMENT;
...
#[cfg(test)]
use dimension_types::bytesrepr::Bytes;
use dimension_types::{
...
static DEPLOY: Lazy<Deploy> = Lazy::new(|| {
    ...
    let payment = ExecutableDeployItem::StoredContractByName {
        name: String::from("dimension-example"),
    ...
    let header = DeployHeader {
        ...
        chain_name: String::from("dimension-example"),
    };
...
impl Deploy {
    ...
    pub fn random_with_timestamp_and_ttl(
        ...
        let chain_name = String::from("dimension-example");
...
impl From<Deploy> for DeployItem {
    fn from(deploy: Deploy) -> Self {
        ...
        DeployItem::new(
            ...
            deploy.header().gas_price(),
            authorization_keys,
            dimension_types::DeployHash::new(deploy.id().inner().value()),
        )
    }
}
...
mod tests {
    ...
    use dimension_execution_engine::core::engine_state::MAX_PAYMENT_AMOUNT;
    use dimension_types::{bytesrepr::Bytes, CLValue};
