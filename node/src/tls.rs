...
///
/// The common name of the certificate will be "dimension-node".
pub fn generate_node_cert() -> SslResult<(X509, PKey<Private>)> {
    let private_key = generate_private_key()?;
    let cert = generate_cert(&private_key, "dimension-node")?;
...
fn generate_cert(private_key: &PKey<Private>, cn: &str) -> SslResult<X509> {
    ...
    let issuer = mkname("US", "Dimension Blockchain", cn)?;
